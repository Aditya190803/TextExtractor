...
export default class TextExtractorExtension extends Extension {
  ...
  _ensureScreenshotDir() {
    ...
    this._screenshotDir = GLib.build_filenamev([
      picturesDir,
      "Screenshots",
      "TextExtractor",
    ]);
    ...
    if (!dir.query_exists(null)) {
      try {
        ...
        console.error(
          `[TextExtractor] Failed to create directory: ${e.message}`,
        );
      }
    }
  }
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        if (!Main.screenshotUI) {
          reject(new Error(_("GNOME Shell screenshot UI is unavailable")));
          return;
        }
      ...
      this._hideLoadingIndicator();
      this._isProcessing = false;
      this._showNotification(_("Screenshot Error"), e.message);
    }
  }
  ...
  _shouldUseShellFallback(error) {
    ...
    return (
      message.includes("org.freedesktop.DBus.Error.UnknownMethod") ||
      message.includes(
  ...
  async _processScreenshotUri(result) {
    ...
    this._showLoadingIndicator(_("Extracting text..."));
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error(
            `[TextExtractor] Failed to delete screenshot: ${deleteErr.message}`,
          );
        }
      } catch (copyErr) {
        console.error(
          `[TextExtractor] Failed to copy screenshot: ${copyErr.message}`,
        );
    ...
    if (!finalPath) {
      throw new Error(_("No valid screenshot path found after capture"));
    }
  ...
  async _takePortalScreenshot() {
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        signalId = Gio.DBus.session.signal_subscribe(
          ...
          (
            ...
            try {
              ...
              console.error(
                `[TextExtractor] Error parsing response: ${e.message}`,
              );
              reject(e);
            }
          },
        ...
        this._portalTimeoutId = GLib.timeout_add_seconds(
    ...
    try {
      ...
      if (!ocrHelperPath) {
        ...
        this._showNotification(
          _("OCR Error"),
          _(
            "text-extractor-ocr is not installed. Re-run install.sh to install the helper and Tesseract automatically.",
          ),
        );
        return;
      }
      ...
      const [stdout, stderr] = await new Promise((resolve, reject) => {
        this._subprocess.communicate_utf8_async(
          null,
          this._cancellable,
          (subprocess, result) => {
            try {
              const [, stdoutStr, stderrStr] =
        ...
        try {
          ...
          if (ocrResult.success) {
            ...
            if (allText.trim()) {
              ...
              this._showNotification(
                _("Text Extracted"),
                _("%d line(s), %d characters copied to clipboard").format(
                  lineCount,
                  charCount,
                ),
              );
              ...
              this._showNotification(
                _("No Text Found"),
                _("No text detected in the selected area"),
              );
            }
          } else {
            ...
            this._showNotification(
              _("OCR Error"),
              ocrResult.error || _("OCR processing failed"),
            );
          }
        } catch (parseError) {
          console.error(
            `[TextExtractor] Failed to parse OCR result: ${parseError.message}`,
          );
          this._showNotification(
            _("OCR Error"),
            _("Failed to parse OCR results"),
          );
        }
      } else {
        console.error(
          `[TextExtractor] OCR produced no output. stderr: ${stderr}`,
        );
        this._showNotification(
          _("OCR Error"),
          stderr || _("OCR produced no output"),
        );
      }
      ...
      this._hideLoadingIndicator();
      this._isProcessing = false;
      this._showNotification(_("OCR Error"), e.message);
    }
  }
  ...
  _cleanupScreenshot(imagePath) {
    ...
    if (this._settings && !this._settings.get_boolean("save-screenshots")) {
      try {
        ...
        console.error(
          `[TextExtractor] Failed to delete screenshot: ${e.message}`,
        );
      }
    }
  }
